...
// Cấu hình Gemini API
const GEMINI_API_KEY = 'AIzaSyB-vqzjj-4lejpeoF5zFTLc23o-8Jf_Qhs'; // API key từ file gốc
...
function isFileTranslated(filePath) {
    try {
        ...
        const langCode = path.dirname(filePath).split('/').pop();
        return data.lang === langCode && data.lang_name === LANGUAGE_NAMES[langCode];
    } catch (error) {
        return false;
    }
}
...
async function main() {
    ...
    console.log('🚀 Bắt đầu dịch các file home.json từ tiếng Anh...');
    ...
    let totalFiles = LANGUAGES.length;
    ...
    for (const lang of LANGUAGES) {
        processedFiles++;
        console.log(`\n[${processedFiles}/${totalFiles}] Processing language: ${lang}...`);
        
        // Kiểm tra file đã tồn tại và đã được dịch chưa
        const targetFilePath = sourceFilePath.replace('/en/home/', `/${lang}/home/`);
        
        if (fs.existsSync(targetFilePath)) {
            if (isFileTranslated(targetFilePath)) {
                console.log(`⏭️  Skipped ${lang}: Already translated`);
                skippedCount++;
                continue;
            } else {
                console.log(`🔄 Re-translating ${lang}: File exists but not properly translated`);
            }
        }
        ...
        // Delay để tránh rate limit (3 giây giữa các request)
